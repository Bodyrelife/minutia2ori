function [input_blobs, random_scale_inds] = Generate_minibatch(conf, image_roidb)
...
%     [im_blob, im_scales] = get_image_blob(conf, image_roidb, random_scale_inds);
    im_blob = zeros(image_roidb.H,image_roidb.W,3);
    for i = 1:size(image_roidb.boxes,1)
        im_blob(image_roidb.boxes(i,2),image_roidb.boxes(i,1),:) = image_roidb.boxes(i,3);
    end
    
    for i = 1:num_images
%         [labels, label_weights, bbox_targets, bbox_loss] = ...
%             sample_rois(conf, image_roidb, fg_rois_per_image, rois_per_image, im_scales(i), random_scale_inds(i));
%         
%         % get fcn output size
%         img_size = round([image_roidb.H image_roidb.W]  * im_scales(i));
...
%         
%         labels_blob = reshape(labels, 1, output_size(1), output_size(2));
%         label_weights_blob = reshape(label_weights, 1, output_size(1), output_size(2));
%         bbox_targets_blob = reshape(bbox_targets', 4, output_size(1), output_size(2));
%         bbox_loss_blob = reshape(bbox_loss', 4, output_size(1), output_size(2));
%         
...
%     assert(~isempty(bbox_loss_blob));
    ...
    input_blobs = {im_blob, labels_blob, label_weights_blob};
end
...
function [labels, label_weights, bbox_targets, bbox_loss_weights] = ...
    ...
    bbox_loss_weights(fg_inds, 1:2) = 1;
end
