function [image_roidb, bbox_means, bbox_stds] = TY_proposal_prepare_image_roidb(conf, image_roidb, bbox_means, bbox_stds)
...
%     parfor i = 1:num_images
    for i = 1:num_images
       ...
       [anchors, im_scales] = proposal_locate_anchors(conf, [image_roidb_cell{i}.H image_roidb_cell{i}.W]);
        
       gt_rois = image_roidb_cell{i}.boxes;
%        gt_labels = image_roidb_cell{i}.class;
       gt_labels = ones(size(gt_rois,1),1);
       im_size = [image_roidb_cell{i}.W image_roidb_cell{i}.H];
...
%   negative: [-1, zero(regression_label)]
    ...
    ex_gt_overlaps = pointdis(ex_rois, gt_rois);
    
    % drop anchors which run out off image boundaries, if necessary
    if conf.drop_boxes_runoff_image
         contained_in_image = is_contain_in_image(ex_rois, round([image_roidb.W image_roidb.H] * im_scale));
         ex_gt_overlaps(~contained_in_image, :) = inf;
    end

    % for each ex_rois(anchors), get its max overlap with all gt_rois
    [ex_max_overlaps, ex_assignment] = min(ex_gt_overlaps, [], 2);
    ...
    [gt_max_overlaps, gt_assignment] = min(ex_gt_overlaps, [], 1);
    ...
    fg_inds = unique([find(ex_max_overlaps <= conf.fg_thresh * im_scale); gt_best_matches]);
    ...
    % pursue high recall
    bg_inds = setdiff(find(ex_max_overlaps < conf.bg_thresh_hi * im_scale & ex_max_overlaps >= conf.bg_thresh_lo * im_scale), ...
                    fg_inds);
    ...
    [regression_label] = TY_fast_rcnn_bbox_transform(src_rois, target_rois);
