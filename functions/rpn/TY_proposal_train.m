function save_model_path = TY_proposal_train(conf, imdb_train, roidb_train, varargin)
...
%     ip.addParamValue('roidb_val',           struct(),           @isstruct);
    ...
    ip.addParamValue('val_iters',           420,                @isscalar);
...
%         if ~mod(iter_,100)
%             sum(sum(abs(caffe_solver.net.params('conv1',1).get_diff()),4),3)
%         end
...
%         shuffled_inds = shuffled_inds(:, randperm(size(shuffled_inds, 2)));
        shuffled_inds = randperm(floor(length(image_roidb_train)/ ims_per_batch) * ims_per_batch);
        shuffled_inds = reshape(shuffled_inds, ims_per_batch, []);
