function [im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_blob] = TY_fast_rcnn_get_minibatch(conf, image_roidb)
...
function [labels, num_classes, rois, bbox_targets, bbox_loss_weights] = ...
    sample_rois(conf, image_roidb, fg_rois_per_image, rois_per_image)

    rois = image_roidb(1).boxes;
    gt_inds = find(image_roidb(1).gt == 1);
...
%     dis(gt_inds,:) = 0;
    ...
    labels = ones(length(keep_inds),1);
    % Clamp labels for the background ROIs to 0
    labels((fg_rois_per_this_image+1):end) = 0;
%     overlaps = overlaps(keep_inds);
    rois = rois(keep_inds, :);
    tempa = rois(:,1);tempb = rois(:,2);
    rois(:,1) = tempa-15;rois(:,2) = tempb-15;
    rois(:,3) = tempa+16;rois(:,4) = tempb+16;
%     rois(:,1) = tempa-31;rois(:,2) = tempb-31;
%     rois(:,3) = tempa+32;rois(:,4) = tempb+32;
...
%        bbox_targets(ind, targets_inds) = bbox_target_data(ind, 1:4);
%        bbox_loss_weights(ind, targets_inds) = 1;  
              ind = inds(i);
       cls = clss(ind);
       targets_inds = (1+cls*4):((cls+1)*4);
       bbox_targets(ind, targets_inds) = bbox_target_data(ind, 2:end);
       bbox_loss_weights(ind, targets_inds(1:end-1)) = 1; 
       bbox_loss_weights(ind, targets_inds(end-1)) = 3;
    end
end
