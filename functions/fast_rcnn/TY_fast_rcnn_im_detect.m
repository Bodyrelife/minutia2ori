function [pred_boxes, scores] = TY_fast_rcnn_im_detect(conf, caffe_net, im, boxes, max_rois_num_in_gpu)
...
% --------------------------------------------------------
    ...
    boxes(:,1) = centerx-15; boxes(:,2) = centery-15;
    boxes(:,3) = centerx+16; boxes(:,4) = centery+16;
%     boxes(:,1) = centerx-31; boxes(:,2) = centery-31;
%     boxes(:,3) = centerx+32; boxes(:,4) = centery+32;
    ...
    scores = scores(:, 2:end);
    box_deltas = box_deltas(:, 5:end);
    
%     pred_boxes = boxes;
    pred_boxes = TY_fast_rcnn_bbox_transform_inv(boxes, box_deltas);
%    pred_boxes = clip_boxes(pred_boxes, size(im, 2), size(im, 1));
    ...
    pred_boxes(:,2) = (pred_boxes(:,2)+pred_boxes(:,4))/2;
    pred_boxes(:,3) = (box_deltas(inv_index,3)+1).*pi;
