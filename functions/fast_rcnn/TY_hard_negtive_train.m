function save_model_path = TY_hard_negtive_train(conf, imdb_train, roidb_train, varargin)
...
%% inputs
    ...
    ip.addParamValue('imdb_val',               @iscell);
    ip.addParamValue('roidb_val',              @iscell);
    ...
    ip.addParamValue('cache_name',      'Zeiler_conv5', ...
                                                        @isstr);
    ip.addParamValue('hditer',                    1,    @isscalar);
...
%% try to find trained model
    ...
    save_model_path = [cache_dir, '/final', num2str(opts.hditer)];
...
%     num_classes = size(image_roidb_train(1).overlap, 2);
    ...
    fprintf('Preparing training data...');
    [image_roidb_train, bbox_means, bbox_stds] = TY_fast_rcnn_prepare_image_roidb(conf, image_roidb_train);
    fprintf('Done.\n');    
    if opts.do_val
        fprintf('Preparing validation data...');
        [image_roidb_val] = TY_fast_rcnn_prepare_image_roidb(conf, image_roidb_val, bbox_means, bbox_stds);
        fprintf('Done.\n');
        % fix validation data
        shuffled_inds_val = generate_random_minibatch([], image_roidb_val, conf.ims_per_batch);
        while (length(shuffled_inds_val) < opts.val_iters)
        	opts.val_iters = floor(opts.val_iters/1.5);
        end
        shuffled_inds_val = shuffled_inds_val(randperm(length(shuffled_inds_val), opts.val_iters));
    end
...
%         rois = image_roidb_train{i}.boxes;
%         bbox_targets = zeros(size(rois, 1), 5, 'single');
%         bbox_gt = image_roidb_train{i}.bbox_targets;
%         distA = zeros(size(rois,1),length(bbox_gt));
%         angleA = distA;
%         ex_inds = false(size(rois,1),1);
%         gt_inds = 1:size(bbox_gt,1);
%         for j=1:size(bbox_gt,1);
%             distAt = abs(rois(:,1:2)-repmat(bbox_gt(j,1:2),size(rois,1),1));
%             distA(:,i) = sum(distAt.').';
%             angleA(:,i) = abs(rois(:,3)-repmat(bbox_gt(j,3),size(rois,1),1));
%             ex_inds = ex_inds|(distA(:,i)<2 & angleA(:,i)<pi/6);
%         end
%         if ~sum(ex_inds)            
%             ex_rois = rois(ex_inds, 1:4);
%             [~,gtassign] = min(distA(ex_inds,:).');
%             gt_rois = bbox_gt(gt_inds(gtassign),:);
%             [regression_label] = fast_rcnn_bbox_transform(ex_rois, gt_rois);
%             bbox_targets(ex_inds, :) = [ones(sum(ex_inds),1), regression_label];
%         end
%         image_roidb_train{i}.boxes = [[image_roidb_train{i}.bbox_targets,ones(size(bbox_gt,1),1)];image_roidb_train{i}.boxes];
%         image_roidb_train{i}.bbox_targets =  [[ones(size(bbox_gt,1),1),zeros(size(bbox_gt,1),4)];bbox_targets]; 
%     end
%     
%     for i = 1:length(image_roidb_val)
%         rois = image_roidb_val{i}.boxes;
%         bbox_targets = zeros(size(rois, 1), 5, 'single');
%         bbox_gt = image_roidb_val{i}.bbox_targets;
%         distA = zeros(size(rois,1),length(bbox_gt));
%         angleA = distA;
%         ex_inds = false(size(rois,1),1);
%         gt_inds = 1:size(bbox_gt,1);
...
%             bbox_targets(ex_inds, :) = [ones(sum(ex_inds),1), regression_label];
%         end
%         image_roidb_val{i}.boxes = [[image_roidb_val{i}.bbox_targets,ones(size(bbox_gt,1),1)];image_roidb_val{i}.boxes];
%         image_roidb_val{i}.bbox_targets =  [[ones(size(bbox_gt,1),1),zeros(size(bbox_gt,1),4)];bbox_targets]; 
%     end
%     fprintf('Done.\n');
%     num_classes = 2;
...
%% training
    ...
    while (iter_ < max_iter)
        ...
        [im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_weights_blob] = ...
            TY_fast_rcnn_get_minibatch(conf, image_roidb_train(sub_db_inds));
        ...
        if ~mod(iter_, opts.val_interval) 
            if opts.do_val
                ...
                for i = 1:length(shuffled_inds_val)
                    ...
                    [im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_weights_blob] = ...
                        TY_fast_rcnn_get_minibatch(conf, image_roidb_val(sub_db_inds));
    ...
    % final snapshot
    snapshot(caffe_solver, bbox_means, bbox_stds, cache_dir, sprintf('iter_%d', iter_));
    save_model_path = snapshot(caffe_solver, bbox_means, bbox_stds, cache_dir, ['final' num2str(opts.hditer)]);
...
%         shuffled_inds = shuffled_inds(:, randperm(size(shuffled_inds, 2)));
        shuffled_inds = reshape(randperm(floor(length(image_roidb_train)/ims_per_batch)*ims_per_batch),ims_per_batch,[]);
