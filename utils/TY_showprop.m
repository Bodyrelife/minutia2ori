function TY_showprop(pic, pos, pos_t)
    dis = pointdis2(pos,pos_t);
    [diss , disidx] = min(dis , [] ,2);
    label = diss<15;
    if numel(label)==0
        figure(1);
        hold off
...
%         temp = Rotate' * Arrow + repmat(pos_t(i, 1:2)', [1, size(Arrow, 2)]);
        plot(pos_t(i, 1), pos_t(i, 2), 'MarkerSize',10,'Marker','o', 'Color', 'g','LineWidth',1.5);
    end
    ...
    for i = 1:size(pos, 1)
        ...
        if label(i) == 1
            color = 'b';
            plot([pos(i,1), pos_t(disidx(i),1)],[pos(i,2), pos_t(disidx(i),2)],'Color','k','LineWidth',3);
        else
            color = 'r';
        end
%         alpha = pos(i, 3);
...
%         temp = Rotate' * Arrow + repmat(pos(i, 1:2)', [1, size(Arrow, 2)]);
        plot(pos(i, 1), pos(i, 2), 'MarkerSize',10,'Marker','o', 'Color', color,'LineWidth',1.5);
    end
    set(gcf, 'PaperPositionMode', 'auto');
%     print(1, '-dbmp', save);
end
