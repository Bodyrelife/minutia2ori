...
tic;
%打开训练图像集
Files = dir('D:\work\FVC2000\DB1_A_Img2\*.bmp');  %打开文件夹中所有图像
inputDir = 'D:\work\FVC2000\DB1_A_Img2\';
% outputDir='C:\Users\helloworld\Desktop\work\medfilt_tmp\';
outputDir='D:\work\FVCori\';
mkdir(outputDir);
% mkdir([outputDir 'Img2\']);
% parpool;
for k = 1:length(Files);
    Filename = [inputDir , Files(k).name];
    Img  = imread(Filename);
    outputname=[outputDir, 'FVC2000_DB1_A_', Files(k).name(1:end-4)];
    fft_enhance_cubs(Img, outputname);
%     view_orientation_image(oimg);
%     imwrite(Img2,[outputDir 'Img2\' Files(k).name],'bmp');
end
% delete('gcp');
toc;
...
tic;
%打开训练图像集
Files = dir('D:\work\FVC2000\DB2_A_Img2\*.bmp');  %打开文件夹中所有图像
inputDir = 'D:\work\FVC2000\DB2_A_Img2\';
% outputDir='C:\Users\helloworld\Desktop\work\medfilt_tmp\';
outputDir='D:\work\FVCori\';
mkdir(outputDir);
% mkdir([outputDir 'Img2\']);
...
for k = 1:length(Files);
    Filename = [inputDir , Files(k).name];
    Img  = imread(Filename);
    outputname=[outputDir, 'FVC2000_DB2_A_', Files(k).name(1:end-4)];
    fft_enhance_cubs(Img, outputname);
%     view_orientation_image(oimg);
%     imwrite(Img2,[outputDir 'Img2\' Files(k).name],'bmp');
end
...
%--------------------------------------------------------------------------
function fft_enhance_cubs(img,outputname)
    global NFFT;
    NFFT        =   32;     %size of FFT
...
%     enhimg      =   zeros(nHt,nWt);
    ...
    for i = 0:nBlkHt-1
        ...
        for j = 0:nBlkWt-1
            ...
            if i>0&&i<nBlkHt-1&&j>0&&j<nBlkWt-1
                blkout=uint8(img(nRow-OVRLP-BLKSZ:nRow+2*BLKSZ+OVRLP-1,nCol-OVRLP-BLKSZ:nCol+2*BLKSZ+OVRLP-1));
            end
            %remove dc
            ...
            if i>0&&i<nBlkHt-1&&j>0&&j<nBlkWt-1
                imwrite(blkout,[outputname, '_', num2str(i*nBlkWt+j+1), '_', num2str(round(oimg(i+1,j+1)*10)/10), '.bmp'],'bmp'); %precision 0.1
            end
        end;%for j
    end;%for i
